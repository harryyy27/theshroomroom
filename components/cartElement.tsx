...
export default function CartElement({_id,idx,name,quantity,price,fresh,size,stripeProductId,stripeId,stockAvailable}:any){
    ...
    return(
        <div className={`cart-wrapper ${stockAvailable<quantity?"cart-out-of-stock":""}`}>
            ...
            <p className="product-price col-3">{!isSale?<span>£{quantity*price}</span>:<><span style={{textDecoration:"line-through"}}>£{Number(quantity*price).toFixed(2)}</span><span style={{marginLeft:"0.25rem",color:"red",fontWeight:"700"}}>£{Number(quantity*price*0.9).toFixed(2)}</span></>}</p>
